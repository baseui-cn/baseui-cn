...
import { type EditableDataGridColumn, useEditableDataGrid } from "@/hooks/use-editable-data-grid"
import { Badge } from "@/components/ui/badge"
import { DataGrid, type DataGridColumnEditor } from "@/components/data-grid/data-grid"
import { DataGridColumnHeader } from "@/components/data-grid/data-grid-column-header"
import { DataGridEditableCell, DataGridEditDialog } from "@/components/data-grid/data-grid-editing"
...
const initialData: TeamMember[] = [
  {
    ...
    active: true,
    rating: 4,
    notes: "Owns the onboarding flows and keeps the design system examples polished.",
  },
  {
    id: "tm-2002",
    name: "Jordan Lee",
    ...
    active: true,
    rating: 5,
    notes: "Maintains the registry pipeline and performance-sensitive interaction examples.",
  },
  {
    id: "tm-2003",
    name: "Naomi Cruz",
    ...
    active: true,
    rating: 5,
    notes: "Pairs closely with contributors and reviews the open source roadmap every week.",
  },
  {
    id: "tm-2004",
    name: "Ethan Brooks",
    ...
    active: false,
    rating: 3,
    notes: "Helping stabilize async demos and improve keyboard accessibility across the grid.",
  },
  {
    id: "tm-2005",
    name: "Camila Stone",
    ...
    active: true,
    rating: 4,
    notes: "Focuses on visual QA and the component catalog presentation language.",
  },
  {
    id: "tm-2006",
    name: "Micah Patel",
    ...
    active: true,
    rating: 4,
    notes: "Owns table-heavy patterns and keeps the data-grid examples aligned across bases.",
  },
]
...
function ActionsCell({ row, table }: { row: Row<TeamMember>; table: Table<TeamMember> }) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        render={
      ...
      <DropdownMenuContent side="bottom" align="start">
        <DropdownMenuItem onClick={() => table.options.meta?.editing?.openDialog(row.original)}>
          Edit via API form
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
...
export function PatternEdit2() {
  ...
  const simulateServerMutation = useCallback(async () => {
    ...
    if (failNextSave) {
      setFailNextSave(false)
      throw new Error("Simulated API failure. Rollback applied. Retry the change.")
    }
  }, [failNextSave])
  ...
  const columns = useMemo<ColumnDef<TeamMember>[]>(
    () => [
      ...
      },
      {
        ...
        id: "workMode",
        header: ({ column }) => <DataGridColumnHeader title="Work Mode" column={column} />,
        cell: ({ cell }) => <DataGridEditableCell cell={cell} />,
        size: 260,
        enableSorting: false,
        meta: {
          skeleton: cellSkeletons.workMode,
          cellClassName: "justify-center px-4",
          editor: {
            kind: "radio",
            label: "Work Mode",
      ...
      },
      {
        accessorKey: "salary",
        id: "salary",
        header: ({ column }) => <DataGridColumnHeader title="Salary" column={column} />,
        cell: ({ cell }) => <DataGridEditableCell cell={cell} />,
        size: 220,
        meta: {
          skeleton: cellSkeletons.salary,
          cellClassName: "justify-center px-0",
          editor: {
            kind: "number",
            label: "Salary",
            min: 60000,
      ...
      },
      {
        accessorKey: "mentor",
        id: "mentor",
        header: ({ column }) => <DataGridColumnHeader title="Mentor" column={column} />,
        cell: ({ cell }) => <DataGridEditableCell cell={cell} />,
        size: 110,
        enableSorting: false,
        meta: {
          ...
          editor: {
            kind: "checkbox",
            label: "Mentor",
            description: "Mark whether this teammate is available for contributor pairing.",
          },
        },
      },
      {
        ...
        meta: {
          ...
          editor: {
            kind: "switch",
            label: "Active",
            description: "Controls whether the teammate is currently active in the rotation.",
          },
        },
      },
      {
        ...
        meta: {
          ...
          editor: {
            kind: "rating",
            label: "Quality score",
            description: "A quick internal confidence score for demo readiness.",
          },
        },
      },
      {
  ...
  const editorColumns = useMemo(
    () =>
      columns.reduce<EditableDataGridColumn<TeamMember>[]>((accumulator, column) => {
        if (column.id && column.meta?.editor) {
          accumulator.push({
            id: String(column.id),
            editor: column.meta.editor as DataGridColumnEditor<TeamMember, unknown>,
          })
        }
  ...
  const editing = useEditableDataGrid<TeamMember>({
    ...
    onError: (_error, context) => {
      add({
        title: "Update failed",
        description: "The optimistic update was rolled back. Retry the change when you are ready.",
        type: "error",
      })
    },
  })
