...
function Command({ items, placeholder = "Type a command or search...", emptyText = "No results found.", className, onSelect }: CommandProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "ArrowDown") { e.preventDefault(); setActiveIndex(i => Math.min(i + 1, flatItems.length - 1)) }
      ...
      if (item) { item.onSelect?.(); onSelect?.(item) }
    }
  }

  return (
    <div className={cn("flex h-full w-full flex-col overflow-hidden rounded-lg bg-popover text-popover-foreground", className)} onKeyDown={handleKeyDown}>
  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      ...
      <DialogPrimitive.Portal>
        <DialogPrimitive.Backdrop className="fixed inset-0 z-50 bg-black/50 data-[open]:opacity-100 data-[closed]:opacity-0 data-[starting-style]:opacity-0 transition-opacity duration-200" />
        <DialogPrimitive.Popup className={cn(
          ...
          "data-[open]:opacity-100 data-[open]:scale-100 data-[open]:-translate-x-1/2",
