...
function ToastList() {
  ...
  return (
    <ToastPrimitive.Viewport className="fixed bottom-4 right-4 z-100 flex flex-col gap-2 w-full max-w-sm outline-none pointer-events-none">
      {toasts.map((toast) => (
        <ToastPrimitive.Root
          key={toast.id}
          toast={toast}
          className={cn(
            ...
            toast.type === "success"
              ...
              : toast.type === "error"
                ...
                : toast.type === "warning"
                  ? "bg-background border-yellow-500/30 text-foreground"
