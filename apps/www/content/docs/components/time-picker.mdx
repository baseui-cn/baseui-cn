...
When opened empty, the picker can show a draft time. OK commits the visible draft; Cancel restores the previous value. Use `defaultPickerValue` to choose that initial draft, or omit it to use the nearest valid stepped current time. Unlike `defaultValue`, it does not commit a value until the user confirms.
...
| `defaultPickerValue`  | `TimeValue`                          | current time  | Initial uncommitted draft when opened empty  |
...
Accepts the TimePicker selection props plus `label`, `placeholder`, `formatPreset`, `displayFormat`, `defaultPickerValue`, `clearable`, `closeOnSelect`, `open`, `defaultOpen`, and `onOpenChange`. It defaults to list mode.
