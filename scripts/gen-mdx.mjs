...
const componentConfig = {
  ...
  command: {
    description: "A command palette composed from Base UI dialog and autocomplete primitives.",
    notes: "Keep `command` as one installable and compose the richer variants from the exported primitives.",
    props: [
      { name: "Command", type: "component", required: true, desc: "Inline command surface built on Autocomplete" },
      { name: "CommandDialog", type: "component", desc: "Overlay root for modal command palettes" },
      { name: "CommandDialogPopup", type: "component", desc: "Dialog popup shell with backdrop and viewport wiring" },
      { name: "CommandInput", type: "component", desc: "Search input with built-in command styling" },
      { name: "CommandList", type: "component", desc: "Scrollable result list for grouped or flat commands" },
      { name: "CommandItem", type: "component", desc: "Selectable command row" },
    ],
  },
  "empty-state": {
  ...
  login: {
    description: "A card-based sign-in block built with Base UI form primitives, alerts, and toast success states.",
    props: [
      { name: "title", type: "string", desc: "Card heading override" },
      { name: "description", type: "string", desc: "Supporting copy below the heading" },
      { name: "showSocial", type: "boolean", defaultValue: "true", desc: "Shows Google and GitHub actions when callbacks are provided" },
      { name: "onSubmit", type: "(values) => AuthSubmitResult | Promise<AuthSubmitResult | void> | void", desc: "Submit handler for credential auth" },
      { name: "variant", type: '"default" | "bordered" | "elevated" | "minimal"', defaultValue: '"default"', desc: "Card presentation style" },
      { name: "size", type: '"sm" | "md" | "lg"', defaultValue: '"md"', desc: "Controls spacing and control sizing" },
    ],
  },
  signup: {
    description: "A card-based signup block with password rules, inline validation, alerts, and toast success states.",
    props: [
      { name: "title", type: "string", desc: "Card heading override" },
      { name: "description", type: "string", desc: "Supporting copy below the heading" },
      { name: "showSocial", type: "boolean", defaultValue: "true", desc: "Shows Google and GitHub actions when callbacks are provided" },
      { name: "onSubmit", type: "(values) => AuthSubmitResult | Promise<AuthSubmitResult | void> | void", desc: "Submit handler for signup" },
      { name: "termsHref", type: "string", desc: "Terms of service link target" },
      { name: "privacyHref", type: "string", desc: "Privacy policy link target" },
    ],
  },
}
